...
argsets["production"] = (
    [
        ...
        "elasticnet",
        "--trainfile",
        data_file("Inputs", "fulltrain.csv"),
        "--testfile",
        data_file("Inputs", "final.csv"),
        "--predictions",
        data_file("Final", "final1.csv"),
        "--no_score",
    ],
    [
        "--tune",
...
def get_parser():
    ...
    parser.add_argument(
        "--trainfile",
        "-T",
        default=data_file("Inputs", "train.csv"),
        help="file to train classifier on",
    )
    parser.add_argument(
        "--testfile",
        "-t",
    ...
    parser.add_argument(
        ...
        "-p",
        default=None,
        help="destination for predictions (or None for default location)",
    )
    parser.add_argument(
        "--logfile", "-l", default=log_file("insults.log"), help="name of logfile"
    )
    parser.add_argument(
        "--tune", "-tu", action="store_true", help="if set, causes tuning step to occur"
    )
    ...
    parser.add_argument(
        ...
        "-sp",
        default="elasticnet",
        help="l1 or l2 or elasticnet (default: %{default}s)",
    )
    ...
    parser.add_argument(
        ...
        "-c",
        action="store_true",
        help="make predictions for the final stage of the production",
    )
    parser.add_argument(
        "--comptune", "-ct", action="store_true", help="tuning for final stage"
    )
    parser.add_argument(
        "--score",
        "-sc",
        action="store_true",
        dest="score",
        help="turn on print out of score at end",
        default=True,
    )
    parser.add_argument(
        "--no_score",
        ...
        action="store_false",
        dest="score",
        help="turn off print out of score at end",
    )
...
def score():
    ...
    print(
        pandas.DataFrame(data, columns=("fn", "score", "public", "private")).sort(
            "score"
        )
    )
