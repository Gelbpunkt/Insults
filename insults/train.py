...
def save_estimates(se):
    ...
    pandas.DataFrame(dict(submission=submissions, estimate=estimates)).to_csv(
        estimate_file, index=False
    )
...
def tuning(arguments):
    ...
    with ProcessPoolExecutor(max_workers=NUM_CORES) as executor:
        future_to_fold = dict(
            [
                (executor.submit(tune_one_fold, arguments, i, train_i, test_i), i)
                for i, (train_i, test_i) in enumerate(boolean_folds)
            ]
        )
...
def predict(folds, arguments):
    ...
    ypred = clf.predict(
        test_examples.Comment
    )  # use the trained classifier to classify comments

    submission = pandas.DataFrame(
        dict(Insult=ypred, Comment=test_examples.Comment, Date=test_examples.Date),
