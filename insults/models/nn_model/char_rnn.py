...
from insults.nn_model.util import (
    setup_logging,
    LossHistory,
    binarize,
    binarize_outshape,
)
from insults.nn_model.plumbing import (
    load_data,
    load_insults_data,
    build_examples_with_their_targets,
)
from insults.nn_model.plumbing import (
    sentence_count_per_doc,
    charset,
    chars_to_indices_vec,
)
...
comments, targets = build_examples_with_their_targets(
    insults_data.Comment, insults_data.Insult
)
...
X_train, X_test, y_train, y_test = dataset_split(
    X, y, train_end=20000, test_start=20000
)
...
check_cb = keras.callbacks.ModelCheckpoint(
    checkpoint_location, monitor="val_loss", verbose=0, save_best_only=True, mode="min"
)
...
model.fit(
    ...
    epochs=5,
    shuffle=True,
    callbacks=[earlystop_cb, check_cb, history],
)
