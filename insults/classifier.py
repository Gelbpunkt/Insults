...
class InsultsSGDRegressor(linear_model.SGDRegressor):
    ...
    def __init__(
        self, n_iter_per_step=50, max_iter=500, alpha=0.001, penalty="l2", **kwargs
    ):
        ...
        for i in range(0, self.max_iter, self.n_iter):
            ...
            if self.coef_ is not None:
                ...
                linear_model.SGDRegressor.fit(
                    self, X, y, coef_init=self.coef_, intercept_init=self.intercept_
                )
            else:
                linear_model.SGDRegressor.fit(self, X, y)
            # record coefs and intercept for later
            self.stages_.append(
                (i + self.n_iter, self.coef_.copy(), self.intercept_.copy())
            )
    ...
    def staged_predict(self, X):
        ...
        """
        return [
            (n_iter_, self.predict(X, coef=coef_, intercept=intercept_))
            for (n_iter_, coef_, intercept_) in self.stages_
        ]
